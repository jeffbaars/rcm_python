...
#---------------------------------------------------------------------------
def get_ensmean_std(file_plot, var, models, years, season, \
    ...
    for d in range(len(dts_all)):
        ...
        for n in range(len(files_c)):
            ...
            if n == 0:
                ...
                variances = np.ones((nx, ny, nt)) * np.nan
            dat_all[:,:,:,n] = plotdat

        for nt in range(ntimes):
            dat_c = np.squeeze(dat_all[:,:,nt,:])
            variances[:,:,nt] = np.std(dat_c, axis=2)

        if d == 0:
            varmean = np.ones((nx, ny, len(dts_all))) * np.nan
        varmean[:,:,d] = np.mean(variances, axis=2)
