...
# Load wrfout file.
#---------------------------------------------------------------------------
#def sum_wrfout_slice(files, vars, maski, maskj, sums, cnts):
...
#    for var in vars:
#        print var, (sums[var] / float(cnts[var]))
#    sys.exit()
...
def get_title(season, stn, var, stat, years):
    titleout = seasons_lab[season] + ' ' + labels[var+stat] + ', ' + \
...
#---------------------------------------------------------------------------
def get_extract_files(data_dirs, model, sdt_yyyymm, edt_yyyymm):
    files_out = []
    for d in range(len(data_dirs)):

#        print 'looking for ', model, ' in ', data_dirs[d]

        path_c = data_dirs[d] + '/' + model + '/extract/*' + '.nc'
#        print 'path_c = ', path_c
        
#        files = glob.glob(data_dirs[d] + '/' + model + '*' + '.nc')
...
#        print 'files = ', files
        for f in range(len(files)):
            ...
            if (dt_c):
                dt_c = dt_c[0]
                yyyymm_c = dt_c[0:6]
#                print 'comparing ', yyyymm_c, ' to ', sdt_yyyymm, ' - ', \
#                      edt_yyyymm
...
#    plt.plot(otot, alpha=0.6, label='Observed', color='black', \
...
#             linewidth=lw_sm)
    ...
    for m in range(len(models)):
        ...
        plt.plot(ptot, alpha = 0.25, label = mod.upper(), color = cols[m], \
                 linewidth=lw_sm)
...
#            files = get_extract_files(data_dir_c, model, sdt, edt)
            files = get_extract_files(data_dirs, model, sdt, edt)            
