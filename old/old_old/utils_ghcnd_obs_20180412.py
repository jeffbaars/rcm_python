...
#                out_min[season,stn,yyyy] = np.nan
            ...
            if (mm == '03' or mm == '04' or mm == '05'):
                ...
                if ((season,stn,yyyy) in out_sum):
                    out_sum[season,stn,yyyy] = out_sum[\
                            season,stn,yyyy] + dat_c
                else:
            ...
            if (mm == '12'):
                ...
                if (season+yyyyw in ndays):
                    ...
                    ndays[season+yyyyw] = 1
                if ((season,stn,yyyyw) in out_sum):
                    out_sum[season,stn,yyyyw] = out_sum[\
                        season,stn,yyyyw] + dat_c
                else:
                    out_sum[season,stn,yyyyw] = dat_c

                #--- Get maximum.
                if ((season,stn,yyyyw) in out_max):
                    if (dat_c > out_max[season,stn,yyyyw]):
                        out_max[season,stn,yyyyw] = dat_c
                else:
                    out_max[season,stn,yyyyw] = dat_c
...
def get_season_summaxmin(dat_c, season, yyyy, ndays, out_sum, out_max, out_min):
    #--- Count number of days for this year's spring.
    if (season+yyyy in ndays):
        ndays[season+yyyy] += 1
    else:
