...
for m in range(len(models)):
    ...
    for y in range(len(years)):
        ...
        if (win_files[yyyy] == win_files[yyyy]):
            test = run_mapper(vardat, 'T2MAX', model, rundir, 'win', \
                              'Temperature (K)', lat, lon, levs_c, my_cmap)
        sys.exit()
...
#            titlein = model.upper() + \
#                      ', Average Max Temperature (K), Winter (DJF) ' + yyyy
#            plotdat = vardat[var][:,:,0]
#            for z in range(len(zooms)):
#                plotfname = rundir + '/win_' + var + '_' + yyyy + '_' + \
#                            zooms[z] + '.png'
