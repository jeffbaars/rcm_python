...
#    plt.plot(smooth(otot,smooth_fact), linestyle='None', marker='o', \
#             markersize=3, label=lab_c, color='black')
    plt.plot(smooth(otot,smooth_fact), marker='o', \
             markersize=3, label=lab_c, color='black')
    plt.plot(otot, alpha=0.4, label='Observed', color='black', \
...
def load_extract_data(geo_em, stns, latpts, lonpts, elevs, models, \
                      data_dir, sdt, edt, vars, pickle_dir, load_pickle):
