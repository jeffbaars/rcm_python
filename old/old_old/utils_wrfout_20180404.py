...
#---------------------------------------------------------------------------
def ts_mods(statplot, years, stn, models, titlein, plotfname, var, \
            stat, cols, seasonsplot):
    ...
    for m in range(len(models)):
        ...
        for y in range(len(years)):
            ...
            for ss in range(len(seasons)):
                ...
                if (season in seasonsplot):
                    if (var == 'PREC'):
                        ptot.append(statplot[season,models[m],stn,yyyy] * mm2in)
                    elif (var == 'T2MAX' or var == 'T2MIN'):
                        ptot.append(statplot[season,models[m],stn,yyyy] -273.15)
                    elif (var.find('SPD') >= 0):
                        ptot.append(statplot[season,models[m],stn,yyyy])
                    if (m == 0):
                        xlabs.append(season.title() + ' ' + years[y])
